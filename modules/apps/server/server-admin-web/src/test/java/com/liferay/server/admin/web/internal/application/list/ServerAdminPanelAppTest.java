...
public class ServerAdminPanelAppTest {
	...
	public void testGetPanelAppNavigationItems() throws Exception {
		...
		for (int i = 0; i < panelAppNavigationItems.size(); i++) {
			...
			Assert.assertEquals(tabs1Name, panelAppNavigationItem.getLabel());
			...
			Assert.assertTrue(
				href,
				href.contains("mvcRenderCommandName=%2Fserver_admin%2Fview") ||
				href.contains("mvcRenderCommandName=/server_admin/view"));
		}
	}

	@Test
	public void testGetPanelAppNavigationItemsHaveDistinctHrefs()
		throws Exception {
		...
		for (PanelAppNavigationItem panelAppNavigationItem :
			...
			Assert.assertTrue(
				...
				hrefs.add(panelAppNavigationItem.getHref()));
		}
	}
