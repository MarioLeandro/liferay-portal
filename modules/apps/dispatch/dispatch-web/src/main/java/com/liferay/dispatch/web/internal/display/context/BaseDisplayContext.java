...
public abstract class BaseDisplayContext {
	...
	public List<NavigationItem> getNavigationItems() {
		...
		return TransformUtil.transform(
			...
			tab -> NavigationItemBuilder.setActive(
				tabs1.equals(tab.getTabs1Name())
			).setHref(
				liferayPortletResponse.createRenderURL(), "tabs1",
				tab.getTabs1Name(), "mvcRenderCommandName",
				tab.getMVCRenderCommandName()
			).setLabel(
				LanguageUtil.get(httpServletRequest, tab.getLabelKey())
			).build());
	}
